...
module Wilde.ApplicationConstruction.ObjectTypeTest
       (
         theTest,
       )
       where
...
theTest :: Test
theTest = TestLabel "ApplicationObjectModelToolsTest" $
          TestList
          [
            test_word32
          , test_double
...
thisError expectedType uInput (Right (Left err@(_,actualType,_))) =
  ...
  where
    errMsg = "Expected error of type " ++ show expectedType ++
...
standardTestsOfMandatoryAndOptional :: (Eq a,Show a)
                                    => (AttributeUiInputResultChecker a
                                        -> [UiI.ElementValue]
                                        -> Test)
